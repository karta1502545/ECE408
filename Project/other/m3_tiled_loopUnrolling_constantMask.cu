...
/*
Conv-GPU==
Layer Time: 341.617 ms
Op Time: 6.51638 ms
Conv-GPU==
Layer Time: 276.829 ms
Op Time: 32.6322 ms

Conv-GPU==
Layer Time: 299.021 ms
Op Time: 6.71471 ms
Conv-GPU==
Layer Time: 253.991 ms
Op Time: 33.0882 ms
*/
    ...
    for (int c=0; c<C; c++) {
        ...
        for (int kr=0; kr<K; kr+=2) {
            for (int kc=0; kc<K; kc+=2) {
                ...
                //     pValue += in_2d(ty*S+kr, tx*S+kc+1) * mask_4d(m, c, kr, kc+1);
                // if (kr < K && kc+2 < K)
                //     pValue += in_2d(ty*S+kr, tx*S+kc+2) * mask_4d(m, c, kr, kc+2);
                ...
                //     pValue += in_2d(ty*S+kr+1, tx*S+kc+1) * mask_4d(m, c, kr+1, kc+1);
                // if (kr+1 < K && kc+2 < K)
                //     pValue += in_2d(ty*S+kr+1, tx*S+kc+2) * mask_4d(m, c, kr+1, kc+2);
                
                // if (kr+2 < K && kc < K)
                //     pValue += in_2d(ty*S+kr+2, tx*S+kc) * mask_4d(m, c, kr+2, kc);
                // if (kr+2 < K && kc+1 < K)
                //     pValue += in_2d(ty*S+kr+2, tx*S+kc+1) * mask_4d(m, c, kr+2, kc+1);
                // if (kr+2 < K && kc+2 < K)
                //     pValue += in_2d(ty*S+kr+2, tx*S+kc+2) * mask_4d(m, c, kr+2, kc+2);
            }
        }
        __syncthreads();
    }
