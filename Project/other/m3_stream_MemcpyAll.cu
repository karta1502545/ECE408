...
Conv-GPU==
Layer Time: 391.223 ms
Op Time: 0.000726 ms
Conv-GPU==
Layer Time: 605.012 ms
Op Time: 0.000687 ms
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    for (int i=0; i<numOfStream; i++) {
        ...
        cudaMemcpyAsync((*device_input_ptr) + inputOffset, host_input + inputOffset, H * W * C * B * sizeof(float), cudaMemcpyHostToDevice, stream[i]);

        conv_forward_kernel<<<DimGrid,DimBlock,0,stream[i]>>>((*device_output_ptr), (*device_input_ptr), *device_mask_ptr, B, M, C, H, W, K, S);
